	...
	window.onload = () => {
		...
		*/
		function initializeFlicktySlider($sliderEl, opts) {
			...
			/* Get the max height of slides and set the height of all to that */
			...
			console.log('this slider: '+$newSlider);
		...
		pageSliders.forEach(($thisSlider) => {
			...
			/*if ($thisSlider.querySelectorAll('.bc-flickty-slider__slide--video').length > 0) {
				const $videoSlide = $thisSlider.querySelectorAll('.bc-flickty-slider__slide--video')[0];
				const $videoSlideWrap = $thisSlider.querySelectorAll('.bc-flickty-slider__video-wrap')[0];
		...
		/* 
			Photo gallery - works with Modula Image Gallery plugin
			https://wp-modula.com/
		*/
		function destroyLightboxSlider($lightbox) {
			...
			if ($slider.querySelectorAll('.bc-flickty-slider__slide').length > 0) {
				Array.from($slider.querySelectorAll('.bc-flickty-slider__slide')).forEach((elm) => {
					elm.remove();
				});
			}
		}
		function initializeLightboxSlider($lightbox, slides) {
			...
			//initializeFlicktySlider($slider, sliderOpts);
		...
		if (Array.from(document.querySelectorAll('.modula.modula-gallery')).length > 0) {
			...
			imageGalleries.forEach(($thisGallery)=> {
				...
				$galleryItems.forEach(($thisItem) => {
					$thisItem.addEventListener('click', (evt) => {
						evt.preventDefault();
						const sliderOpts = {
							cellSelector: '.bc-flickty-slider__slide',
							prevNextButtons: false,
							pageDots:  true,
							adaptiveHeight: true,
							initialIndex: $galleryItems.findIndex((el) => evt.currentTarget.isSameNode(el)),
							contain: true
						};
						...
						$closeLightbox.addEventListener('click', (evt) => {
							evt.preventDefault();
		...
		/*
			Hero video controls - 
			Adds play/pause capability to hero video
		*/
		if (document.querySelector('.bc-hero__video')) {	
			...
			$videoControls.addEventListener('click', (evt) => {
				evt.preventDefault();
				let duration = 0.182;
				if ($videoWrapper.classList.contains('.is-paused') && $heroVideo.paused) {
					$heroVideo.play();
					$bc.gsap.to($videoPlayIcon, {opacity: 0, duration: duration}).eventCallback('onComplete', () => {
					...
					$heroVideo.pause();
					$bc.gsap.to($videoPauseIcon, {opacity: 0, duration: duration}).eventCallback('onComplete', () => {
		...
		/*
			Custom - handles elements which have waves: adds a negative margin top on nextSiblings to pull the sibling up under the wave
			Then increasing the top padding of the next sibling to compensate for that. 
